...
- **Run cadence is operator-controlled.** `agent.run_interval_hours` (1-24, default 1 = hourly) mirrors the CI schedule. CI's `discover.yml` fires hourly and a gate step compares the UTC hour against the `OPENRADAR_RUN_HOURS` repo variable (default 1) — intervals > 1 run only when the hour is a multiple. The **Settings → Agent → Run now** button triggers an on-demand run on the local server (single-flight, 10-minute cap, log to `data/.run.log`); the GitHub Actions **Run workflow** button is the equivalent for CI.
